...
if (!class_exists('M_SysAdmin')) {
    class M_SysAdmin
    {
        ...
        public function deactivateEmployee($id) {
            $this->db->query('UPDATE employee SET status = :status WHERE employee_id = :id');
        ...
        // Update searchEmployees to include status filter
        public function searchEmployees($search = '', $branchId = '', $status = '') {
            $sql = 'SELECT 
                        e.employee_id, 
                        e.full_name, 
                        e.address, 
